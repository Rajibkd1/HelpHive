...
<body class="bg-gray-50">
    <div class="max-w-6xl mx-auto p-4 sm:p-6 mt-12 lg:p-8">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600">Ticket Details</h1>
                <p class="text-gray-500 mt-1">View and manage support ticket information</p>
            </div>
            <a href="{{ route('tickets') }}" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-300 ease-in-out">
                <i class="fas fa-arrow-left mr-2"></i> Back to Tickets
            </a>
        </div>

        <!-- Ticket Information with improved styling -->
        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden mb-8 ticket-card hover:shadow-xl transition-all">
            <!-- Ticket Header -->
            <div class="bg-gradient-to-r from-blue-600 to-purple-600 p-6 text-white">
                <div class="flex justify-between items-start">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-lg bg-white bg-opacity-20 backdrop-filter backdrop-blur-sm flex items-center justify-center text-white font-bold text-xl shadow-inner">
                        ...
                        @foreach($ticket->uploads as $upload)
                            ...
                            <li class="group relative">
                                @if(in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg']))
                                    <div class="w-48 h-48 overflow-hidden rounded-lg shadow-md hover:shadow-lg transition-all">
                                        <img src="{{ $filePath }}" alt="{{ $fileName }}" class="w-full h-full object-cover attachment-image cursor-pointer" onclick="toggleFullscreen(this)">
                                    </div>
                                    <div class="absolute bottom-2 left-2 right-2 bg-black bg-opacity-60 text-white text-xs p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity text-center truncate">
                                        {{ $fileName }}.{{ $extension }}
                                    </div>
                                @elseif($extension == 'pdf')
                                    <a href="{{ $filePath }}" class="flex flex-col items-center p-4 bg-white border rounded-lg shadow-md hover:shadow-lg transition-all w-48 h-48" target="_blank">
                                        <i class="fas fa-file-pdf text-red-500 text-4xl mb-2"></i>
                                        <span class="text-sm text-gray-700 text-center truncate w-full">{{ $fileName }}.{{ $extension }}</span>
        ...
        <div class="flex justify-center mb-8">
            <button id="replyButton" class="px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-full shadow-md hover:shadow-lg transition-all duration-300 flex items-center gap-2">
                ...
                <div class="bg-white p-6 rounded-xl shadow-md ticket-response border border-gray-100">
                    <div class="flex justify-between items-center mb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded-lg bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold text-xl shadow-md">
                                {{ substr($response->agent->name ?? 'A', 0, 1) }}
                            </div>
                            <div>
                                <h3 class="font-semibold text-gray-900 text-lg">{{ $response->agent->name ?? 'Agent' }}</h3>
                        ...
                        <div class="mt-4">
                            @if(in_array($respExtension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg']))
                                <div class="mt-3 p-2 bg-gray-50 rounded-lg inline-block">
                                    ...
                                    <img src="{{ $respFilePath }}" alt="{{ $respFileName }}" class="max-w-md rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer" onclick="toggleFullscreen(this)">
                                </div>
                            @elseif($respExtension == 'pdf')
                                <a href="{{ $respFilePath }}" class="mt-3 flex items-center gap-2 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors inline-block" target="_blank">
                                    <i class="fas fa-file-pdf text-red-500 text-xl"></i>
                                    <span class="text-blue-500 hover:underline">{{ $respFileName }}.{{ $respExtension }}</span>
                                </a>
                            @else
                                <a href="{{ $respFilePath }}" class="mt-3 flex items-center gap-2 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors inline-block" target="_blank">
